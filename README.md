...
| tool | what it does |
|---|---|
| `session_create` | Create a named session: a new forum topic + a fresh agent session; returns the conversation `key` and the `model` it runs. Takes an optional `backend` (`zcode` or `codex`) and, for `codex` only, an optional `model` — see below. Without `chat_id` the target is **auto-picked**: the forum-enabled groups the bot knows (Topics enabled, bot-admin preferred, most recently used first) — see below. |
