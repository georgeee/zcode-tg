// Unit test for bridge/codexClient.js's stdin write-guard: a write that
...
// test/fixtures/fake-codex-app-server.mjs with FIXTURE_CODEX_CLOSE_STDIN,
...
// than racing the fixture's boot. The fake is spawned directly (no
// interposed node), exactly as CodexClient spawns the real binary.
//
// Run: node --test test/codex-client.test.js
...
import { CodexClient } from '../bridge/codexClient.js';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const FIXTURE = path.join(HERE, 'fixtures', 'fake-codex-app-server.mjs');
...
test('a write the dead app-server can never read fails the call fast -- never an uncaughtException', async (t) => {
  const dir = mkdtempSync(path.join(tmpdir(), 'codex-client-'));
  ...
  const client = new CodexClient({
    ...
    codexHome: dir,
    cwd: dir,
    env: { FIXTURE_CODEX_CLOSE_STDIN: closedMarker }, // the fixture closes its stdin read end at startup
  });
  client.start();
  try {
    ...
    let settled = false;
    const call = client.call('initialize', {}).then(
    ...
    await assert.rejects(client.call('thread/start', {}), /exited .* before responding/);
