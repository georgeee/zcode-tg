...
export function createMcpGateway({ port, unixSocket, host = '127.0.0.1', log = () => {} }) {
  ...
  //
  // Every case below coerced with String(args.x), and String(undefined) is
  // "undefined" -- a perfectly valid string that flows on as if the caller
  // had meant it. Measured live on the Mochi fleet (2026-09-18): a senior
  // model began sending session_create {title: "..."} instead of {name}, and
  // the bridge created seven consecutive forum topics whose Telegram title
  // AND stored name were both the literal "undefined". They are
  // indistinguishable in the topic list, and this bridge wraps no Telegram
  // method that renames a forum topic, so undoing it needed a human in the
  // Telegram UI.
  //
  ...
  async function callTool(name, args) {
    ...
    switch (name) {
      case 'session_create':
        return handlers.sessionCreate(requiredString('session_create', 'name', args.name), args.chat_id != null ? Number(args.chat_id) : undefined);
      case 'session_close':
